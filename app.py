...
@st.cache_datadef load_data():return pd.read_csv("data.csv")

df = load_data()

DATA CLEANING

df.fillna(df.median(numeric_only=True), inplace=True)df["rent"] = df["rent"].clip(df["rent"].quantile(0.01), df["rent"].quantile(0.99))

FEATURE ENGINEERING

df["bath_per_bed"] = df["bathrooms"] / (df["beds"] + 1)df["room_density"] = df["area"] / (df["beds"] + 1)df["bed_bath_ratio"] = df["beds"] / (df["bathrooms"] + 1)df["area_per_room"] = df["area"] / (df["beds"] + df["bathrooms"] + 1)df["locality_target"] = df.groupby("locality")["rent"].transform("mean")

MODEL TRAINING

@st.cache_resourcedef train_models(data):
...
col1.markdown(f"Total Property Listings{len(df)}", unsafe_allow_html=True)col2.markdown(f"Number of Cities{df['city'].nunique()}", unsafe_allow_html=True)col3.markdown(f"Average Rental Price₹{int(df['rent'].mean())}", unsafe_allow_html=True)
...
st.markdown("---")st.markdown("Academic Project (PT-2) | Indian House Rent Prediction System",unsafe_allow_html=True)
